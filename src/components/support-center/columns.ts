...
export type TicketTableRow = Ticket & { searchText: string }
...
export const createSupportTicketColumns = (handlers: {
  onView: (row: TicketTableRow) => void
  onEdit: (row: TicketTableRow) => void
  onSetStatus: (row: TicketTableRow, status: TicketStatus) => void
  onDelete: (row: TicketTableRow) => void
}): ColumnDef<TicketTableRow>[] => [
  {
    id: 'number',
    enableSorting: false,
    enableHiding: true,
  ...
  },
  {
    ...
    header: 'Type',
    cell: ({ row }) =>
      h(Badge, { variant: 'outline', class: getTicketTypeBadgeClass(row.original.type) }, () =>
        ticketTypeLabelMap[row.original.type],
      ),
  },
  {
    id: 'category',
  ...
  },
  {
    ...
    header: 'Status',
    cell: ({ row }) =>
      h(Badge, { variant: 'outline', class: getTicketStatusBadgeClass(row.original.status) }, () =>
        ticketStatusLabelMap[row.original.status],
      ),
  },
  {
    ...
    header: 'Priority',
    cell: ({ row }) =>
      h(Badge, { variant: 'outline', class: getTicketPriorityBadgeClass(row.original.priority) }, () =>
        ticketPriorityLabelMap[row.original.priority],
      ),
  },
  {
    id: 'actions',
      ...
      h('div', { class: 'flex justify-end' }, [
        h(RowActionsMenu, {
          ...
          onSelect: (key: string) => {
            ...
            if (key === 'in-progress') handlers.onSetStatus(row.original, 'in-progress')
            if (key === 'resolved') handlers.onSetStatus(row.original, 'resolved')
