...
export const createScannerColumns = (handlers) => [
    ...
    { accessorKey: 'location', header: 'Location' },
    {
        ...
        header: 'Description',
        cell: ({ row }) => h('span', { class: 'block max-w-[320px] truncate', title: row.original.description }, row.original.description || '-'),
    },
    {
        accessorKey: 'state',
        header: 'State',
        cell: ({ row }) => h(Badge, { variant: 'outline', class: getScannerStateBadgeClass(row.original.state) }, () => formatState(row.original.state)),
    },
        ...
        cell: ({ row }) => h('div', { class: 'flex justify-end' }, [
            h(RowActionsMenu, {
                ...
                onSelect: (key) => {
                    ...
                    if (key === 'working')
                        handlers.onSetState(row.original, 'working');
                    if (key === 'faulty')
                        handlers.onSetState(row.original, 'faulty');
                    if (key === 'test')
                        handlers.onSetState(row.original, 'test');
                    if (key === 'copy')
                        handlers.onCopy(row.original);
                    if (key === 'delete')
