...
export type UserTableRow = User & { searchText: string }
...
export const createUserColumns = (handlers: {
  ...
  onToggle: (row: UserTableRow) => void
  ...
  },
  {
    ...
    header: 'Email',
    cell: ({ row }) =>
      h('span', { class: 'font-mono text-xs' }, row.original.email),
  },
  {
    accessorKey: 'role',
    header: 'Role',
    cell: ({ row }) =>
      h(Badge, { variant: 'outline', class: getUserRoleBadgeClass(row.original.role) }, () => roleLabelMap[row.original.role]),
  },
  {
      ...
      h('div', { class: 'flex justify-end' }, [
        h(RowActionsMenu, {
          ...
          onSelect: (key: string) => {
            ...
            if (key === 'reset') handlers.onReset(row.original)
