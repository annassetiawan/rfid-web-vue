...
export type LocalRequestTableRow = LocalRequest & { searchText: string }
...
export const createLocalRequestColumns = (handlers: {
  onView: (row: LocalRequestTableRow) => void
  onEdit: (row: LocalRequestTableRow) => void
}): ColumnDef<LocalRequestTableRow>[] => [
  {
    id: 'number',
    enableSorting: false,
    enableHiding: true,
