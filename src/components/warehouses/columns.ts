...
import type { Warehouse } from '@/mock/warehouses'

export type WarehouseTableRow = Warehouse & { searchText: string }
...
export const createWarehouseColumns = (handlers: {
  onView: (row: WarehouseTableRow) => void
  onEdit: (row: WarehouseTableRow) => void
  onToggle: (row: WarehouseTableRow) => void
  onDelete: (row: WarehouseTableRow) => void
}): ColumnDef<WarehouseTableRow>[] => [
  {
    id: 'number',
    enableSorting: false,
    enableHiding: true,
  ...
  { accessorKey: 'code', header: 'Code' },
  {
    ...
    header: 'Address',
    cell: ({ row }) =>
      h('span', { class: 'block max-w-[320px] truncate', title: row.original.address }, row.original.address),
  },
