...
import type { CycleCountRecord } from '@/mock/cycleCount'

export type CycleCountTableRow = CycleCountRecord & {
  searchText: string
}

export const createCycleCountColumns = (handlers: {
  onView: (row: CycleCountTableRow) => void
  onStart: (row: CycleCountTableRow) => void
  onClose: (row: CycleCountTableRow) => void
}): ColumnDef<CycleCountTableRow>[] => [
  {
    id: 'number',
    enableHiding: true,
    enableSorting: false,
      ...
      h('div', { class: 'flex justify-end' }, [
        h(RowActionsMenu, {
          ...
          onSelect: (key: string) => {
            ...
            if (key === 'start') handlers.onStart(row.original)
            if (key === 'close') handlers.onClose(row.original)
          },
        }),
      ]),
  },
