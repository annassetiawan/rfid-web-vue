...
export type InventoryStatus = 'In Stock' | 'In Transit' | 'Reserved' | 'Assigned'
...
export function useInventoryTable() {
  ...
  const columns = ref<InventoryColumn[]>([
  ...
  const visibleColumns = reactive<Record<InventoryColumnKey, boolean>>({
    name: true,
    serialNumber: true,
    rfidCode: true,
  ...
  const filteredRows = computed(() => {
    ...
    return rows.value.filter((row) => {
      ...
      if (filters.inventoryStatus && row.inventoryStatus !== filters.inventoryStatus) return false
      ...
      if (filters.stagingStatus && row.stagingStatus !== filters.stagingStatus) return false
      if (filters.warehouseLocation && row.warehouseLocation !== filters.warehouseLocation) return false
      if (filters.dateFrom && row.taggedDate < filters.dateFrom) return false
      if (filters.dateTo && row.taggedDate > filters.dateTo) return false
